...
fuck=np.arange(0,2.5,0.5)
fuck2=np.arange(1,3.5,0.5)
Acp=np.ones((1,len(B)))[0]*0.25
Acp[0:5]=fuck
Acd=np.ones((1,len(B)))[0]*0.5
Acd[5:10]=fuck
Ae=np.ones((1,len(B)))[0]*0.5
Ae[10:15]=fuck
Apg=np.ones((1,len(B)))[0]
Apg[15:20]=fuck
Ape=np.ones((1,len(B)))[0]
Ape[20:25]=fuck
Aqhnew=np.ones((1,len(B)))[0]
Aqhnew[25:30]=fuck2
Aqgnew=np.ones((1,len(B)))[0]
Aqgnew[30:35]=fuck2
Af=np.ones((1,len(B)))[0]
Af[35:40]=fuck
Af[50]=0.1
ACgpg=np.ones((1,len(B)))[0]*0.3
ACgpg[40:45]=fuck
ACgpg[51]=0.1
ACgpg[55]=0.3
ACgpg[56]=0.4
ACgph=np.ones((1,len(B)))[0]*0.2
ACgph[45:50]=fuck
ACgph[52]=0.1
ACgph[53]=0.1
ACgph[54]=0.2
AChpg=np.ones((1,len(B)))[0]*0.4
AChpg[40:45]=fuck
AChpg[51]=0.1
AChpg[55]=0.3
AChpg[56]=0.4
AChph=np.ones((1,len(B)))[0]*0.1
AChph[45:50]=fuck
AChph[52]=0.1
AChph[53]=0.1
AChph[54]=0.2
A=[str(i)+"kaka" for i in range(0,57)]
for ti in range(54,57):
    name=A[ti]
    np.random.seed(0)
    pehigh =Ape[ti]
    pecanshu =250
    ...
    powerchange = 0.01
    Cnew = 1
    newinvesthot =Aqhnew[ti]
    newinvestgreen = Aqgnew[ti]
    newinvest = 0
