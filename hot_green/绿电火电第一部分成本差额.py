...
ES = np.linspace(0, 2, 40)
GA = np.linspace(0, 2, 40)
ESide = np.arange(0, len(ES))
GAide = np.arange(0, len(GA))
YgdealtoneighborVR = np.zeros((len(GA), 1))
YgdealtopoolVR = np.zeros((len(GA), 1))
YgnotdealVR = np.zeros((len(GA), 1))
YhdealtoneighborVR = np.zeros((len(GA), 1))
YhdealtopoolVR = np.zeros((len(GA), 1))
YhnotdealVR = np.zeros((len(GA), 1))
NgdealtoneighborVR = np.zeros((len(GA), 1))
NgdealtopoolVR = np.zeros((len(GA), 1))
NgnotdealVR = np.zeros((len(GA), 1))
NhdealtoneighborVR = np.zeros((len(GA), 1))
NhdealtopoolVR = np.zeros((len(GA), 1))
NhnotdealVR = np.zeros((len(GA), 1))
department_utilityVR = np.zeros((len(GA), 1))
EeVR = np.zeros((len(GA), 1))
FfVR = np.zeros((len(GA), 1))
for faith in ESide:
    for lina in GAide:
        np.random.seed(0)
        Total_rounds = 20000
