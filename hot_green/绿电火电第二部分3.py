...
for ti in range(len(A)):
    ...
    df8.to_excel('E:\\data\火电绿电2\第二部分\完成配额以可再生能源为主的电力企业与邻居交易人数' + name + '.xlsx')
    df9 = pd.DataFrame(Ygdealtopool)
    df9.to_excel('E:\\data\火电绿电2\第二部分\完成配额以可再生能源为主的电力企业与pool交易人数' + name + '.xlsx')
    df10 = pd.DataFrame(Ygnotdeal)
    df10.to_excel('E:\\data\火电绿电2\第二部分\完成配额以可再生能源为主的电力企业不交易人数' + name + '.xlsx')
    df11 = pd.DataFrame(Yhdealtoneighbor)
    df11.to_excel('E:\\data\火电绿电2\第二部分\完成配额以传统能源为主的电力企业与邻居交易人数' + name + '.xlsx')
    df12 = pd.DataFrame(Yhdealtopool)
    df12.to_excel('E:\\data\火电绿电2\第二部分\完成配额以传统能源为主的电力企业与pool交易人数' + name + '.xlsx')
    df13 = pd.DataFrame(Yhnotdeal)
    df13.to_excel('E:\\data\火电绿电2\第二部分\完成配额以传统能源为主的电力企业不交易人数' + name + '.xlsx')
    df14 = pd.DataFrame(Ngdealtoneighbor)
    df14.to_excel('E:\\data\火电绿电2\第二部分\未完成配额以可再生能源为主的电力企业与邻居交易人数' + name + '.xlsx')
    df15 = pd.DataFrame(Ngdealtopool)
    df15.to_excel('E:\\data\火电绿电2\第二部分\未完成配额以可再生能源为主的电力企业与pool交易人数' + name + '.xlsx')
    df16 = pd.DataFrame(Ngnotdeal)
    df16.to_excel('E:\\data\火电绿电2\第二部分\未完成配额以可再生能源为主的电力企业不交易人数' + name + '.xlsx')
    df17 = pd.DataFrame(Nhdealtoneighbor)
    df17.to_excel('E:\\data\火电绿电2\第二部分\未完成配额以传统能源为主的电力企业与邻居交易人数' + name + '.xlsx')
    df18 = pd.DataFrame(Nhdealtopool)
    df18.to_excel('E:\\data\火电绿电2\第二部分\未完成配额以传统能源为主的电力企业与pool交易人数' + name + '.xlsx')
    df19 = pd.DataFrame(Nhnotdeal)
    df19.to_excel('E:\\data\火电绿电2\第二部分\未完成配额以传统能源为主的电力企业不交易人数' + name + '.xlsx')
    df20 = pd.DataFrame(department_utility)
    df20.to_excel('E:\\data\火电绿电2\第二部分\监管部门效用' + name + '.xlsx')
    df21 = pd.DataFrame(Ee)
    df21.to_excel('E:\\data\火电绿电2\第二部分\监管部门奖励' + name + '.xlsx')
    df22 = pd.DataFrame(Ff)
    df22.to_excel('E:\\data\火电绿电2\第二部分\监管部门惩罚' + name + '.xlsx')
    df23 = pd.DataFrame(greeninpool)
    df23.to_excel('E:\\data\火电绿电2\第二部分\绿证数量供给' + name + '.xlsx')
    df24 = pd.DataFrame(greenprice)
    df24.to_excel('E:\\data\火电绿电2\第二部分\绿证价格' + name + '.xlsx')
    df25 = pd.DataFrame(greenoutpool)
    df25.to_excel('E:\\data\火电绿电2\第二部分\绿证数量需求' + name + '.xlsx')
    df26 = pd.DataFrame(beforced)
    df26.to_excel('E:\\data\火电绿电2\第二部分\被迫进入pool' + name + '.xlsx')
    df27 = pd.DataFrame(Ybeforced)
    df27.to_excel('E:\\data\火电绿电2\第二部分\完成配额被迫进入pool' + name + '.xlsx')
    df28 = pd.DataFrame(Nbeforced)
    df28.to_excel('E:\\data\火电绿电2\第二部分\未完成配额被迫进入pool' + name + '.xlsx')
